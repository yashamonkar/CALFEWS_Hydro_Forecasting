...
shasta_gen = get_shasta_generation(Releases, Storage, shasta_discharge_capacity*31)
...
new_melones_gen = get_new_melones_generation(Releases, Storage, new_melones_discharge_capacity*31)
...
#Compute the Generation 
folsom_gen = get_folsom_generation(Releases, Storage, folsom_discharge_capacity*31)
...
#Compute the Generation 
keswick_gen = get_keswick_generation(Releases, keswick_discharge_capacity*31)
...
#Compute the Generation 
trinity_gen = get_trinity_generation(Releases, Storage, trinity_discharge_capacity*31)
...
release = Folsom_Outflow['VALUE']
storage = Folsom_Storage['VALUE']
hydro_module = get_folsom_generation
penstock_capacity = folsom_discharge_capacity
WY = wy_types['SAC_Index']
eia_gen = eia['Folsom']/1000

def compare_bias_timestep(release, storage, hydro_module, penstock_capacity, WY, eia_gen):
...
# %% ANALYSIS OF BIAS IN FLOOD SPILLS#

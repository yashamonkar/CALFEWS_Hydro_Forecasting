...
columns_before_aug_2022 = ['Day', 'Elev', 'Storage', 'Storage_Change', 'Total_Inflow' , 
                           'Diversion_Inflow','Clear_Creek_Inflow', 'Outlet', 'Spill', 
                           'Spring_Creek_Outlet','Evap_cfs', 'Evap_inches', 'Precip']

columns_after_aug_2022 = ['Day', 'Elev', 'Storage', 'Storage_Change', 'Total_Inflow' ,
                          'Diversion_Inflow','Clear_Creek_Inflow', 'Spill', 'Outlet',
                          'Spring_Creek_Outlet','Evap_cfs', 'Evap_inches', 'Precip']

#Start the loop
for file_pattern in file_patterns:
    filename = f"data/CVP_Reports/Whiskeytown/whidop{file_pattern}.pdf"
...
#Save the file. 
final_df.to_csv('data/Whiskeytown_Daily_Operations.csv')

#_____________________________________________________________________________#
### Initial Visualization of the Diversions. 
fig = plt.figure(figsize=(18,6))
ax = plt.subplot(122)
ax.plot(final_df['Outlet'][0:365], label='Diversions', linestyle='solid')
ax.legend()
ax.set_xlabel('Date')
...
for year in range(2001, 2025):
    ...
    water_year_data = final_df['Clear_Creek_Inflow'][mask]
...
plt.title('Whiskeytown Diversions (WY 2001-2024)', fontsize=14)
plt.xlabel('Month', fontsize=12)
plt.ylabel('Clear_Creek_Inflow (CFS)', fontsize=12)

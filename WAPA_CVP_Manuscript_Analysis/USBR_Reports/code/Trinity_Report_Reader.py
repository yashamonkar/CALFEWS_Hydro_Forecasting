...
columns = ['Day', 'Elev', 'Storage', 'Storage_Change', 'Inflow_CFS',
           'Power', 'Outlet', 'Spill', 'Evap_cfs', 'Evap_inches', 'Precip']


#Start the loop
for file_pattern in file_patterns:
    filename = f"data/CVP_Reports/Trinity/trndop{file_pattern}.pdf"
    ...
    for col in columns:  # Use consistent column list for cleaning
...
#Save the file. 
final_df.to_csv('data/Trinity_Daily_Operations.csv')

#_____________________________________________________________________________#
### Initial Visualization of the Spill. 
fig = plt.figure(figsize=(18,6))
ax = plt.subplot(122)
ax.plot(final_df['Spill'][0:365], label='Spill', linestyle='solid')
ax.legend()
ax.set_xlabel('Date')
ax.set_ylabel('Spill (CFS/day)')

#Compute Total Releases from Trinity
final_df['Total'] = final_df['Power'] + final_df['Outlet'] + final_df['Spill']
final_df['Total'] = final_df['Total']*1.98211/1000 #Convert to TAF/day
...
for year in range(2001, 2025):
    ...
    water_year_data = final_df['Total'][mask]
...
plt.title('Trinity Releases (WY 2001-2024)', fontsize=18)
plt.xlabel('Month', fontsize=16)
plt.ylabel('Releases', fontsize=16)

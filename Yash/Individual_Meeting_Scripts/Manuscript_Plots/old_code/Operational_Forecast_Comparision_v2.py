...
#%% Predictions
#Set up the inflows for the testing period. 
#Option 1: Compute the percentiles in the original cumsum months
...
ax.plot(eia.index, eia['Stat_Gen'], 
        color='violet',  linewidth=2, label='EIA')
...
for i, (month, month_num) in enumerate(zip(months, month_numbers)):
    row = i // 3  # Changed: divide by 3 instead of 4
    col = i % 3   # Changed: modulo 3 instead of 4
    ax = axes[row, col]
    ...
    # Create scatter plot
    if len(x_data) > 0:
        ax.scatter(x_data, y_data, alpha=0.6, color='blue', s=100)
    ...
    if row == 3:  # Bottom row (changed from 2 to 3)
...
# Create 1x2 subplot (bottom row only)
...
# Left: Annual CVP vs CALFEWS
x3 = eia_annual['CVP_Gen']
y3 = eia_annual['CALFEWS_Gen']
r2_3, rmse_3, slope_3, intercept_3 = calculate_stats(x3, y3)
axes[0].scatter(x3, y3, alpha=0.7, color='blue', s=100, label='CALFEWS')
for i, year in enumerate(y3.index):
...
axes[0].set_xlabel('EIA Generation (GWh)', fontsize=24)
axes[0].set_ylabel('Forecast Generation (GWh)', fontsize=24)
axes[0].text(0.5, 0.17, f'R² = {r2_3:.2f}\nRMSE = {rmse_3:.0f} GWh', 
                transform=axes[0].transAxes, verticalalignment='top', fontsize=20,
                bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
axes[0].tick_params(axis='both', labelsize=18)
axes[0].grid(True, alpha=0.3)

# Right: Annual CVP vs Statistical
x4 = eia_annual['CVP_Gen']
y4 = eia_annual['Stat_Gen']
r2_4, rmse_4, slope_4, intercept_4 = calculate_stats(x4, y4)
axes[1].scatter(x4, y4, alpha=0.7, color='red', s=100, label='Statistical')
for i, year in enumerate(y4.index):
...
axes[1].set_xlabel('EIA Generation (GWh)', fontsize=24)
axes[1].set_ylabel(' ', fontsize=24)
axes[1].text(0.5, 0.17, f'R² = {r2_4:.2f}\nRMSE = {rmse_4:.0f} GWh', fontsize=20,
                transform=axes[1].transAxes, verticalalignment='top',
                bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
axes[1].tick_params(axis='both', labelsize=18)
axes[1].grid(True, alpha=0.3)

# Add legend to the right plot only, including both CALFEWS and Statistical entries
blue_patch = plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='blue', markersize=10, label='CALFEWS')
red_patch = plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='red', markersize=10, label='Statistical')

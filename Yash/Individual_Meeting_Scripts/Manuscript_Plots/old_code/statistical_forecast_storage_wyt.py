...
input_fnf = input_fnf['Total_Inflow']

#Add the water year types
...
fig, ax = plt.subplots(figsize=(8, 8))
scatter = ax.scatter(eia['CVP_Gen'], y_pred, 
                    color='blue', s=60, alpha=0.7, label='Observed Data')
slope, intercept, r_value, p_value, std_err = stats.linregress(eia['CVP_Gen'], y_pred)
line_x = np.linspace(eia['CVP_Gen'], y_pred, 100)
line_y = slope * line_x + intercept
min_val = min(eia['CVP_Gen'].min(), y_pred.min())
max_val = max(eia['CVP_Gen'].max(), y_pred.max())
ax.plot([min_val, max_val], [min_val, max_val], 
        'k-', linewidth=1, alpha=0.4)
ax.text(0.05, 0.95, f'R² = {r_value**2:.2f}', 
        transform=ax.transAxes, fontsize=14,
        bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
ax.text(0.05, 0.9, f'RMSE = {np.sqrt(np.mean((eia["CVP_Gen"] - y_pred)**2)):.2f}', 
        transform=ax.transAxes, fontsize=14,
        bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))

# -*- coding: utf-8 -*-
"""
Created on Wed Jun 18 10:49:27 2025
...
for year in range(2004,2024): #Change the value soon 1996 to 2024
    ...
    for all_years in range(1996,2024):
        cvp_gen = pd.read_csv(f"Annual_Ensembles/results/{year}/{all_years}/Daily_CVP_Generation.csv", index_col =0)
        
        # Convert index to datetime if it's not already
        cvp_gen.index = pd.to_datetime(cvp_gen.index)

        # Remove Feb 29 if it exists
        feb_29_mask = (cvp_gen.index.month == 2) & (cvp_gen.index.day == 29)
        cvp_gen = cvp_gen[~feb_29_mask]
...
start_date = '2003-10-01'  #CHANGE CHANGE CHANGE '1995-10-01'
date_index = pd.date_range(start=start_date, periods=total_days, freq='D')
exceedances.index = date_index    
exceedances = exceedances[exceedances.index > pd.Timestamp("2003-09-30")]
...
exceedances = exceedances.drop(['month','WYT', 'WY'], axis=1)
exceedances['year_month'] = exceedances.index.to_period('M')
exceedances = exceedances.groupby('year_month').sum()


#Save the result as CALFEWS forecast
eia['CALFEWS_Gen'] = exceedances['adjusted_gen_v2'].values

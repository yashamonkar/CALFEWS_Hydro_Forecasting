...
def model_attribute_loop_generator(clean_output, modelno, modelso):
  ...
  for reservoir_obj in modelno.reservoir_list + modelso.reservoir_list:
    for key in ['S', 'R', 'R_to_delta', 'available_storage', 'outflow_release', 'days_til_full','contract_flooded', 'fci', 'tocs','diversions', 'restoration',
                'reclaimed_carryover','flood_spill','flood_deliveries','Q','SNPK','downstream','fnf', 'rainflood_fnf', 'snowflood_fnf']:
      try:

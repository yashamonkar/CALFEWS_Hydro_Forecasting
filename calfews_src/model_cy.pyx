...
cdef class Model():
  ...
  cdef tuple northern_initialization_routine(self, scenario='baseline'):
	...
	# self.delta.forecastSTI (self.T x 1) - forecasts for trinity river index (Stored in delta)
  ...
  cdef void initialize_northern_res(self) except *:
    ...
    # Trinity Reservoir System
    self.trinity = Reservoir(self, 'trinity', 'TRT', self.model_mode)

    self.reservoir_list = [self.shasta, self.oroville, self.yuba, self.folsom, self.newmelones, self.donpedro,
                           self.exchequer, self.trinity]
    ...
    reservoir_list = [self.shasta, self.oroville, self.folsom, self.yuba, self.newmelones, self.donpedro, self.exchequer, self.millerton, self.trinity]
  ...
  cdef void initialize_delta_ops(self) except *:
    ...
    inflow_list = [self.shasta, self.folsom, self.yuba, self.oroville, self.newmelones, self.donpedro, self.exchequer, self.trinity]
...
#####################################################################################################################
  ...
  def find_running_WYI(self):
    ...
    index_exceedence = 2
    reservoir_list = [self.shasta, self.folsom, self.oroville, self.yuba, self.newmelones, self.donpedro, self.exchequer, self.millerton] 	# Trinity is not included in the 8 river index. 
...
#####################################################################################################################
  ...
  cdef tuple simulate_north(self, int t, int swp_release, int cvp_release, int swp_release2, int cvp_release2, double swp_pump, double cvp_pump):
    ...
    for reservoir_obj in [self.newmelones, self.donpedro, self.exchequer]:
      reservoir_obj.release_environmental(t, d, m, dowy, y, self.first_d_of_month[year_index], self.delta.forecastSJWYT)
    ...
    for reservoir_obj in [self.shasta, self.oroville, self.yuba, self.folsom]:
      reservoir_obj.release_environmental(t, d, m, dowy, y, self.first_d_of_month[year_index], self.delta.forecastSCWYT)
    #Trinity Restoration Flows
    for reservoir_obj in [self.trinity]:
      reservoir_obj.release_environmental(t, d, m, dowy, y, self.first_d_of_month[year_index], self.delta.forecastSTWYT)
      
    #Compute the Trinity Diversions
    for reservoir_obj in [self.trinity]:
      reservoir_obj.compute_diversion(t, m, y, self.delta.forecastSTI[t], self.trinity.eos_day, cfs_tafd, self.shasta.fcr)
    self.trinity.diversions[t] = self.trinity.consumed_releases + self.trinity.envmin - self.trinity.restoration[t]
    ...
    for reservoir_obj in [self.shasta, self.oroville, self.yuba, self.folsom, self.trinity]:
    ...
    cvp_stored_release = self.shasta.envmin + self.folsom.envmin + self.trinity.consumed_releases #Note Trinity diversion flows are accounted as consumed releases in the trinity book. 
    swp_stored_release = self.oroville.envmin + self.yuba.envmin
    #Tentative Addition (Can be removed)
    trinity_stored_release = self.trinity.envmin
    ...
    #Tentative Addition (Currently not added since Trinity operations are not optimized for pumping at the delta)
    #trinity_available_storage = max(self.trinity.available_storage[t], 0.0)
	...
	#Adding for Trinity
    for reservoir_obj in [self.trinity]:
      reservoir_obj.find_flow_pumping(t, m, dowy, year_index, self.days_in_month, self.dowy_eom, self.delta.forecastSTWYT, 'env')
      reservoir_obj.days_til_full[t] = min(reservoir_obj.numdays_fillup['env'], reservoir_obj.numdays_fillup['lookahead'])  
    ...
    cvp_stored_flow = self.shasta.sodd + self.folsom.sodd + self.trinity.consumed_releases
  ...
  cdef (int, int, int, int, double, double) simulate_south(self, int t, double hro_pump, double trp_pump, double swp_alloc, double cvp_alloc, dict proj_surplus, dict max_pumping, double swp_forgone, double cvp_forgone, double swp_AF, double cvp_AF, double swp_AS, double cvp_AS, str wyt, str wytSC, dict max_tax_free, dict flood_release, dict flood_volume) except *:
    ...
    for reservoir_obj in watershed_reservoir_list:
      reservoir_obj.rights_call(reservoir_obj.downstream[t])
      reservoir_obj.release_environmental(t, d, m, dowy, y, self.first_d_of_month[year_index], wyt)

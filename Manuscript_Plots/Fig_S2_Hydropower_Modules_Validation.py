...
lewiston_ops = pd.read_csv("Manuscript_Plots/data/Lewiston_Daily_Operations.csv", index_col=0)
...
for i, column in enumerate(eia.columns):
    ...
    # Calculate r² and add to panel
    mask = ~(eia[column].isna() | cvp_gen[column].isna())
    r2 = eia[column][mask].corr(cvp_gen[column][mask]) ** 2
    axes[i].text(0.97, 0.95, f'r² = {r2:.2f}', transform=axes[i].transAxes,

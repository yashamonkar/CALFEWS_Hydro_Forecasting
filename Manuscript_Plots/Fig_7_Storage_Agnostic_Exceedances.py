# -*- coding: utf-8 -*-
"""
Created on Mon Apr 28 16:31:22 2025
...
from Manuscript_Plots.post_processing.functions.get_CALFEWS_results import get_results_sensitivity_number_outside_model
from Manuscript_Plots.post_processing.functions.get_comparison_plots import get_comparison_plots
from Manuscript_Plots.post_processing.functions.get_CVP_hydro_gen import get_CVP_hydro_gen
...
plt.fill_between(sim_hydro_dry[0]['CVP_Gen'].index, p10_dry, p90_dry, 
                 color='r', alpha=0.3, label='Low Storage Start Range')
plt.fill_between(sim_hydro_dry[0]['CVP_Gen'].index, p10_wet, p90_wet, 
                 color='b', alpha=0.3, label='High Storage Start Range')
plt.plot(sim_hydro_dry[0]['CVP_Gen'].index, p50_dry,
         'r-', linewidth=2, alpha=1, linestyle = 'dashdot', label='Low Storage Start Median') 
plt.plot(sim_hydro_dry[0]['CVP_Gen'].index, p50_wet,
         'b-', linewidth=2, alpha=1, linestyle = 'dashdot', label='High Storage Start Median')   
ax = plt.gca()
...
water_years = [2016, 2017, 2018, 2019, 2020, 2021]
for i, wy in enumerate(water_years):
    if i < len(water_years) - 1:
        ...
        if i > 1:
            ax.text(mid_point, 1050, f'$\\mathbf{{WY-{i+1}}}$\n (Converges)', ha='center', va='top', fontsize=18)
        elif i > 0 : 
            ax.text(mid_point, 1050, f'$\\mathbf{{WY-{i+1}}}$\n (Nearly \nConverges)', ha='center', va='top', fontsize=18)
        else: 
